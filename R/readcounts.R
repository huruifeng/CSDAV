...
#'  \item{\code{annotation.csv} and \code{annotation.xlsx}, two annotation files, either of which may be chosen to fill out the following sample information: condition, replicate.}
...
read.counts <- function(id=NULL,library=NULL,counts=NULL,annotation=NULL,shinyF=NULL) {
  # Read counts -------------------------------------------------------------------------
  ...
  if(!is.null(library0)) {
    ...
    } else library0 <- library0[,c("sgRNA","Gene")]
  ...
  if(is.null(annotation0)) {
    ...
    annotation0$Replicate <- annotation0$Condition <- ""
  }
  ...
  annotation0 <- annotation0[,c("Sample","Depth","Zeros","Gini","Condition","Replicate")]
