...
if(!"Category"%in%colnames(analyzed))
  analyzed <- data.frame(Gene=analyzed$Gene,Category="All",analyzed[,-1],check.names=FALSE,stringsAsFactors=FALSE)
if(!"P(replicates)"%in%colnames(analyzed))
  analyzed$`P(replicates)` <- 2*pnorm(-abs(analyzed$Z))

largest <- table(analyzed$Category)
...
plot.volcano1 <- function(analyzed) {
  ...
  gg <- ggplot2::ggplot(analyzed, ggplot2::aes(Z,`P(sgRNAs)`,group=Gene,col=Category)) +
    ...
    ggplot2::geom_point(data=analyzed[analyzed$Category==largest,],size=3,pch=1) +
    ggplot2::geom_point(data=analyzed[analyzed$Category!=largest,],size=3,pch=1) +
...
plot.volcano2 <- function(analyzed) {
  ...
  gg <- ggplot2::ggplot(analyzed, ggplot2::aes(Z,`P(replicates)`,group=Gene,col=Category)) +
    ggplot2::xlab("Differential essentiality Z score") +
...
plot.scores <- function(dataset,condition1,condition2) {
  ...
  gg <- ggplot2::ggplot(dataset,ggplot2::aes_string("X","Y",group="Gene",col="Category")) +
